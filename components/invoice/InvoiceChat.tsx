...
export default function InvoiceChat() {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: 'rgba(255,255,255,0.055)', border: '1px solid rgba(255,255,255,0.07)', height: 'calc(100vh - 200px)', minHeight: '500px' }}
    >
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-3">
          ...
          >
            ...
            {msg.role === 'assistant' ? (
              ...
              <div
                ...
                style={{ backgroundColor: user?.avatar_color ?? '#6a5cff' }}
              >
        ...
        {loading && (
          <div className="flex gap-2.5 items-start animate-fadeIn">
            ...
            <div
              className="text-sm text-txt3 px-3.5 py-2.5 rounded-[14px]"
              style={{ backgroundColor: 'rgba(255,255,255,0.055)', border: '1px solid rgba(255,255,255,0.07)' }}
            >
      ...
      >
        <input
          ...
          disabled={loading}
          className="flex-1 bg-raised border border-border rounded-btn px-3 py-2 text-sm text-txt placeholder-txt3 outline-none focus:border-indigo/50 focus:ring-1 focus:ring-indigo/20 transition-all duration-300 disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={loading || !input.trim()}
          className="px-4 py-2 rounded-btn text-white text-sm font-medium transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0"
          style={{ background: 'linear-gradient(135deg, #6a5cff, #3b82f6)' }}
        >
